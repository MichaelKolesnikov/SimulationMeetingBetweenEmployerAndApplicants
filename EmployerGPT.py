...
class EmployerGPT:
    def __init__(self, names: list[str], start_messages: list[str], logging_file_name: str = 'logs.txt'):
        ...
        for i in range(self.n):
            self.write(f"{self.names[i]}:" + start_messages[i] + '\n')
    ...
    def say_first_sentence(self) -> tuple[int, str]:
        ...
        self.write(f"Бот - обращение к {self.names[who]}: {reply}" + '\n')
        return who, reply
    ...
    def answer_on_message(self, person_number: int, message: str) -> tuple[int, str]:
        self.write(f"{self.names[person_number]} : " + message + '\n')
        ...
        self.write(f"Бот - обращение к {self.names[person_number]}: {reply} \n")

        return person_number, reply

    def delete_person(self, person_number: int) -> None:
        self.persons.pop(person_number)
        self.names.pop(person_number)


def main():
    names = ['Jake', "Michael"]
    employer = EmployerGPT(names, ["Здравствуйте", "Добрый день!"], "lol.txt")
